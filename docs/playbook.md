...
- **Swap engine DEFAULT = GPT Image** (`gpt-image-1`), not Flux — it obeys targeted "swap only the man in the middle" edits, which Flux Kontext `/multi` can't reliably do (it doesn't map "first/second image" to array order; keys off subject descriptions). The user's own prompt always leads; the server wraps it (`input_fidelity:high` + orientation `size` + exact-dims resize). An **A/B toggle** runs two enhancement wrappers on the SAME prompt+images and returns both to compare: **A = concise** (prompt + light "keep the rest"), **B = explicit** (enumerated full-transfer from image 2 + everything to preserve). Single runs use B. Flux stays one click away (short BFL-style prompt). Verified 2026-07-09.
