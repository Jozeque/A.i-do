...
- **App implementation (Swap tab).** Full-CHARACTER swap (head to toe + styling), NOT face-only: Flux Kontext `flux-pro/kontext/max/multi` — image 1 = base to keep, image 2 = the character. The prompt forces a whole-character replacement + natural blend (match image 1's light/grade/perspective/grain) + keep everyone & everything else. Output is pinned to image 1's **exact pixel dimensions**: `aspect_ratio` is snapped to image 1's shape, then the result is resized to image 1's W×H via jimp (`imageSize` reads the header, no native deps). GPT Image path adds `input_fidelity:high` + orientation-matched `size`. Dedicated face-swap models (Easel/inswapper) are deliberately NOT used here — they replace only the face/hair, not the full character (and Easel is deprecated on fal). Verified 2026-07-09.
- **Swap engine DEFAULT = GPT Image 2** (`gpt-image-2` — ChatGPT Images 2.0, the exact model the ChatGPT web app uses; released 2026-04-21, GPT-5.4 backbone), not Flux — it obeys targeted "swap only the man in the middle" edits, which Flux Kontext `/multi` can't reliably do (it doesn't map "first/second image" to array order; keys off subject descriptions). NB: the old `gpt-image-1` produced poor swaps — the model version was the main problem. gpt-image-2 does NOT accept `input_fidelity` (natively high-fidelity) and honours `size:auto` by matching the input aspect. ~$0.21/edit at high quality (vs ~$0.06 for gpt-image-1). The user's own prompt always leads; the server wraps it (`quality:high` + `size:auto` + exact-dims resize). An **A/B toggle** runs two enhancement wrappers on the SAME prompt+images and returns both to compare: **A = concise** (prompt + light "keep the rest"), **B = explicit** (enumerated full-transfer from image 2 + everything to preserve). Single runs use B. Flux stays one click away (short BFL-style prompt). Verified 2026-07-09.
