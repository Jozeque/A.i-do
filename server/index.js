...
function compileNbFramesDirection(b) {
  ...
  if (!(campaign || look || lighting || lens || palette || grain || environment || aspectRatio || wardrobe || extra)) return '';
  ...
  if (lens)        out.push(`Camera & lens: ${lens}. Shoot every frame on this exact camera body and lens series and NAME them in the prompt — they are the constant look DNA and never change; only the focal length varies, so pick one specific focal length inside the stated range that fits each shot's framing and depth.`);
...
{"look":"","lighting":"","lens":"","palette":"","grain":"","environment":"","aspectRatio":"","wardrobe":"","extra":""}
...
- grain: read the reference's GRAIN, NOISE, and capture finish precisely and state it to MATCH exactly — this is a big reason the user attached the reference. Is it clean and digitally smooth with minimal grain, a fine natural film grain, a heavier or coarser grain, with any halation, gate weave, or highlight bloom? Give the grain's fineness and strength and any film-stock character so every generated frame carries the SAME capture texture as the reference (e.g. "clean, near-grainless digital finish" or "fine natural 35mm film grain with gentle highlight halation"). Never guess or default to clean — match what the reference actually shows, neither cleaner nor grainier.
...
Describe only what you can actually see; do not invent. Keep values concise.`;
    ...
    const FIELDS = ['look', 'lighting', 'lens', 'palette', 'grain', 'environment', 'aspectRatio', 'wardrobe', 'extra'];
    const builder = {};
