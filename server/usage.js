...
//   Claude Haiku 4.5 — $1.00 / 1M input, $5.00 / 1M output   (text-only chat turns)
//   Claude Sonnet 5  — $3.00 / 1M input, $15.00 / 1M output  (turns WITH attached images;
//     intro pricing $2/$10 ran through 2026-08-31 — the table below is the standard rate)
...
// ~Claude vision tokens per attached image. Haiku caps images at 1568px on the long edge
// (~1600 tokens max); Sonnet 5 reads up to 2576px (~4784 max), so an image turn costs
// substantially more there — price the two separately or vision spend reads ~3x low.
const IMG_IN_TOK = 1500;
const IMG_IN_TOK_VISION = 3500;
...
const GEM_FILES = {
  ...
  'nb-advisor': ['nb-advisor.txt'], 'gpt-advisor': ['gpt-advisor.txt'], 'character-builder': ['character-builder.txt'], 'storyboard': ['storyboard.txt'],
};
let _sysTok = null;
...
// visionModel defaults to claudeModel so a caller that passes only one model (or runs with the
// vision split disabled) prices every turn the same way it always did.
export async function computeUsage(data, claudeModel, visionModel = claudeModel) {
  const sysTok = await gemSysTokens();
  const rate = CLAUDE[claudeModel] || CLAUDE._default;
  const visionRate = CLAUDE[visionModel] || CLAUDE._default;
